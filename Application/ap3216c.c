...
void ap3216c_loop(void)
{
	uint8_t status_int;
uint16_t tmp;
	switch(ap3216c_st.cstate)
	{
		case 0:
			ap3216c_st.nxstate=1;
			ap3216c_write(REG_SYS_CONF, 0x04); //Reset
	break;
		case 1:
			ap3216c_setup();
			ap3216c_st.nxstate=2;
			break;
		case 2:
			//if(int_flag)
			{
				int_flag=false;
			status_int=ap3216c_read(REG_INT_STATUS);
			sprintf(buf_aux,"i: 0x%x",status_int);
			nus_printStr(buf_aux);
			//if(status_int & 0x01)		//ALS INT
			{
				als=ap3216c_read_als_val();
				sprintf(buf_aux,"als = %u",als);
				nus_printStr(buf_aux);

			}
//			if(status_int & 0x02)		//PS INT
			{
				tmp=ap3216c_read16(REG_DATA_PS);
				if(tmp & 0x4000)
				{
					sprintf(buf_aux,"overflow data!");
					nus_printStr(buf_aux);
				}
				//else
				{
					if(tmp & 0x8000)
					{
						sprintf(buf_aux,"Obeject: near");
						nus_printStr(buf_aux);

					}
					else
					{
						sprintf(buf_aux,"Obeject: far");
						nus_printStr(buf_aux);
		...
		case 3:
			break;
		default:
			ap3216c_st.nxstate=0;
			break;
	}


	ap3216c_st.cstate=ap3216c_st.nxstate;
}
